...
La función debe retornar true si los paréntesis están equilibrados y correctamente anidados, y false en caso contrario. 
...
¡Espero que disfrutes resolviendo este desafío!*/

const parentesisCorrectos = (parentesis)=>{
    let result = parentesis.split("").filter((item, index, arr)=> arr.indexOf(item) !== index).length
    return result % 2 === 0 ? true : false
}

console.log(parentesisCorrectos("((a+b) * (c+d))"));
console.log(parentesisCorrectos("(a+b) * (c+d))"));
